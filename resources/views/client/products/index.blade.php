    ...
    @foreach($categories as $category)
        <div class="mb-5">
            <h3 style="font-weight:600;">
                {{ $category->name }}
                <a href="{{ route('client.product.category', $category->slug) }}" style="font-size:14px; color:#09f; margin-left:10px;">
                    Xem tất cả →
                </a>
            </h3>
                ...
                @foreach($category->latestProducts as $p)
                    ...
                    <a href="{{ route('client.product.detail', ['slug' => $p->slug]) }}" 
                       ...
                       style="background:#fff;border-radius:8px;padding:10px;text-align:center;transition:transform .3s;">
                        ...
                        <div class="product-name" style="margin-top:8px;font-weight:600;color:#111">{{ $p->name }}</div>
